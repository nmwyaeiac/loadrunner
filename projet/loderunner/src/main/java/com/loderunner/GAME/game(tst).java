    ...
    while (running) {
      ...
      for (enemy e : gameMap.getEnemy()) {
        if (!e.getIsUnderGround()) {
          ...
          if (next != null) {
            if (next == character.Direction.LEFT) physics.moveLeft(e);
            if (next == character.Direction.RIGHT) physics.moveRight(e);
            if (next == character.Direction.UP) physics.moveUP(e);
            if (next == character.Direction.DOWN) physics.moveDown(e);
          }
        } 
      }
